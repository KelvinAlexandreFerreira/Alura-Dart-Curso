...
class TimeQuestions{
  ...
  timeQuestion() {
    ...
    if (question == hour) {
      ...
      String _botAnswer = kelvinBot + ' Opa! são : ${date.hour} horas e ${date.minute} minutos!';
      print(_botAnswer);
    } else if (question == day) {

      String _botAnswer = kelvinBot + ' Opa! hoje é dia : ${date.day}';
      print(_botAnswer);
    } else if (question == year) {

      String _botAnswer = kelvinBot + ' Opa! estamos em : ${date.year}';
      print(_botAnswer);
    } else {
      unsure();
    }
  ...
  unsure() {
    if (question.contains('horas') || question.contains('Horas')) {
      ...
      String _botQuestion =
          kelvinBot + ' Não entendi, você quer saber das horas?';
      print(_botQuestion);
      String answer = stdin.readLineSync().toString();
      if (answer.contains('Sim') || answer.contains('sim')) {

        String _botAnswer = kelvinBot + ' Beleza, são : ${date.hour} horas e ${date.minute} minutos!';
        print(_botAnswer);
      } else {
      ...
      String _botQuestion =
          kelvinBot + ' Não entendi, você quer saber que dia é hoje?';
      print(_botQuestion);
      String answer = stdin.readLineSync().toString();
      if (answer.contains('Sim') || answer.contains('sim')) {

        String _botAnswer = kelvinBot + ' Beleza, hoje é dia: ${date.day}';
        print(_botAnswer);
      } else {
      ...
      String _botQuestion =
          kelvinBot + ' Não entendi, você quer saber em que ano estamos?';
      print(_botQuestion);
      String answer = stdin.readLineSync().toString();
      if (answer.contains('Sim') || answer.contains('sim')) {
